...
public class MiningJobs {
    ...
    public void processLedger() {
        ...
        if (CollectionUtils.isEmpty(unprocessedLedgers)) return;
        a:
        for (Ledger l : unprocessedLedgers) {
            ...
            if (c == 0) {
                log.info("the ledger table is not exist, ready to create ledger table: ledger_" + l.getPromotionAddress());
            ...
            log.debug("Timing task [process ledger]: the data of ledger[{}] has processed, the content is -- {}", l.getId(), new String(l.getData()));
        }
    }
    ...
    public void cachePromotionLedger() {
        ...
        if (CollectionUtils.isEmpty(promotions)) return;
        for (Promotion p : promotions) {
            try {
                if (1 != promotionLedgerMapper.existTable(p.getAddress())) continue;// exist no table
                List<Ledger> list = promotionLedgerMapper.findLimit(p.getAddress(), promotionLedgerCacheSize);
                JSONArray arr = new JSONArray();
                if (!CollectionUtils.isEmpty(list)) arr.addAll(list);
