...
public class PromotionController {
    ...
    @GetMapping("/page")
    public String page(
            @ApiParam(name = "page", value = "Page number. The first page is 1", type = "int", required = true)
            @RequestParam("page") int page,
            @ApiParam(name = "size", value = "The data size per page, max is 10", type = "int", required = true)
            @RequestParam("size") int size,
            @ApiParam(name = "kw", value = "key word", allowEmptyValue = true, defaultValue = "")
            @RequestParam("kw") String kw
    ) {
        Page<Promotion> p = promotionService.page(page, size, kw.trim());
        List<PromotionInfoVo> d = p.getData().stream().map(t -> BeanUtil.toBean(t, PromotionInfoVo.class)).collect(Collectors.toList());
    ...
    @ApiResponses({@ApiResponse(code = 200, message = "OK", response = PromotionInfoVo.class)})
    @GetMapping("/{address}")
    public String get(@ApiParam(name = "address", value = "promotion address", type = "String", required = true)
    ...
    @ApiResponses({@ApiResponse(code = 200, message = "OK", response = PromotionInfoVo.class)})
    @PutMapping("/{address}")
    public String updateDescription(@ApiParam(name = "address", value = "promotion address", type = "String", required = true)
                                    @PathVariable("address") String address,
                                    @ApiParam(value = "promotion info, only use the field named `introduction` here", type = "json", required = true)
    ...
    public String listWallet(@ApiParam(name = "address", value = "promotion address", type = "String", required = true)
                             @PathVariable("address") String address) {
        if (StringUtils.isBlank(address)) {
            return DataResponse.fail(HttpStatusExtend.ERROR_PROMOTION_NOT_FOUND);
        }
