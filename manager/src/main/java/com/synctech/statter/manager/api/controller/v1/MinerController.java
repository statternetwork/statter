...
public class MinerController {
    ...
    @PostMapping("")
    public String get(@ApiParam(value = "basic machine param", type = "json", required = true) @RequestBody ReqMinerInfo req) {
    ...
    @PostMapping("/wallet")
    public String bindWallet(@ApiParam(value = "bind wallet address param", type = "json", required = true)
                             @RequestBody @Validated Miner miner) {
        ...
        if (!StringUtils.isBlank(m.getWalletAddress())) {// If there is a wallet address, you need to unlock the old wallet address to unbind first
            throw new AppBizException(HttpStatusExtend.ERROR_MINER_HAS_BIND_WALLET);
        } else if (!StringUtils.isBlank(m.getPromotionAddress())) {// If the wallet address is empty, there is a mining pool address, I hope the binding wallet address needs to be under the old mining pool address
            // Compare whether the address of the two is consistent, if it is not consistent, suspend this operation
            ...
            } else if (!((m.getPromotionAddress()).equals(w.getPromotionAddress()))) {// Wallet ore pond address is inconsistent with the mining ponds in the mining machine
    ...
    public String unbindWallet(@ApiParam(name = "sn", value = "machine sn code", type = "String", required = true) @PathVariable("sn") String sn,
        ...
        if (StringUtils.isBlank(m.getWalletAddress())) {
            ...
            throw new AppBizException(HttpStatusExtend.ERROR_MINER_UNBIND_WALLET);
        } else if (!m.isCanPledger()) {// The mining machine is in a state of pledge, indicating that the unpuffed or the redemption is completed
